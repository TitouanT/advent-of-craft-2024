...
from communication.location import Location
...
class SantaCommunicatorTest(unittest.TestCase):
    ...
    def test_compose_message(self):
        message = self.communicator.compose_message("Dasher", Location("North Pole", 5), self.number_of_day_before_christmas)
    ...
    def test_is_overdue_detect_overdue_reindeer(self):
        overdue = self.communicator.is_overdue(
            "Dasher",
            Location("North Pole", self.number_of_day_before_christmas),
    ...
    def test_is_overdue_return_false_when_not_overdue(self):
        overdue = self.communicator.is_overdue(
            "Dasher",
            Location("North Pole", self.number_of_day_before_christmas - self.number_of_days_to_rest - 1),
            self.number_of_day_before_christmas,
            self.logger
        )
